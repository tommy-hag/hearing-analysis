...
def preprocess_and_extract_criticmarkup(md_text: str):
    critic_blocks = []
    original_text = md_text  # Keep reference to original for anchor finding
    ...
    # First pass: Extract {==highlight==}{>>comment<<} blocks
    pattern = re.compile(r'\{==(.*?)==\}\s*\{>>(.+?)<<\}', re.DOTALL)
    md_text = pattern.sub(replacement_func, md_text)
    
    # Second pass: Extract standalone {>>comment<<} blocks without highlights
    # We need to process these in reverse order to preserve positions
    standalone_pattern = re.compile(r'\{>>(.+?)<<\}', re.DOTALL)
    matches = list(standalone_pattern.finditer(md_text))
    ...
    for match in reversed(matches):
        ...
        # Find preceding text anchor - look backwards in the current md_text
        ...
        preceding_text = md_text[line_start:start_pos].strip()
        
        # If we're at start of line (e.g., after heading), use the heading text
        if not preceding_text or preceding_text.endswith('#'):
            # Look for heading on this line
            line_text = md_text[line_start:min(start_pos + 100, len(md_text))]
            heading_match = re.search(r'^#+\s*(.+?)(?:\s*#*)?\s*$', line_text, re.MULTILINE)
            if heading_match:
                anchor = heading_match.group(1).strip()
            else:
                # Fallback: use last word/phrase before comment
                words = re.findall(r'\S+', preceding_text)
                anchor = words[-1] if words else "Kommentar"
        else:
            # Use last word or phrase before comment as anchor
            # Try to get a meaningful anchor (last 1-3 words)
            words = re.findall(r'\S+', preceding_text)
            if len(words) >= 3:
                anchor = ' '.join(words[-3:])
            elif len(words) >= 1:
        ...
        block_index = len(critic_blocks)
        critic_blocks.append({'highlight': anchor, 'comment': comment_content})
        # Replace with placeholder
        md_text = md_text[:match.start()] + f"__CRITIC_MARKUP_{block_index}__" + md_text[match.end():]
    
    return md_text, critic_blocks
